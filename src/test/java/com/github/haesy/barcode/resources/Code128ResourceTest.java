...
@QuarkusTest
public class Code128ResourceTest
{
    private static final String PATH = "/code-128";
    ...
    public void testUmlauts()
    {
        ...
        given().queryParam(CONTENT, "ö").when().get(PATH).then().statusCode(400).header(CONTENT_TYPE, TEXT_PLAIN);
        given().queryParam(CONTENT, "ü").when().get(PATH).then().statusCode(400).header(CONTENT_TYPE, TEXT_PLAIN);
        given().queryParam(CONTENT, "ß").when().get(PATH).then().statusCode(400).header(CONTENT_TYPE, TEXT_PLAIN);
    }
